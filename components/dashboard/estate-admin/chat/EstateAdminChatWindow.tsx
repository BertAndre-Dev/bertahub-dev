...
import { getChatHistory } from "@/redux/slice/estate-admin/chat/estate-admin-chat-thunks";
import ChatHeader from "@/components/chat/ChatHeader";
import EstateAdminChatInput from "@/components/dashboard/estate-admin/chat/EstateAdminChatInput";
...
export default function EstateAdminChatWindow() {
  const dispatch = useDispatch<AppDispatch>();
  const { canCreateChat } = useChatPermissions();

  const activeChat = useSelector((state: RootState) => state.estateAdminChat.activeChat);
  const messages = useSelector((state: RootState) => state.estateAdminChat.messages);
  const pag = useSelector((state: RootState) => state.estateAdminChat.messagesPagination);
  const loading = useSelector((state: RootState) => state.estateAdminChat.messagesLoading);
    ...
    <div className="h-full flex flex-col bg-background rounded-2xl border overflow-hidden">
      ...
      {chatEnded ? (
        <div className="border-t bg-background p-4">
          ...
          <EstateAdminNewChatModal open={newChatOpen} onClose={handleCloseNewChat} />
        </div>
      ) : (
        <EstateAdminChatInput chatId={activeChat._id} />
      )}
    </div>
  );
}
