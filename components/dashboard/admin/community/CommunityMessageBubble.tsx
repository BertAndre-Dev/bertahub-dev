...
import { CheckCheck, CornerDownLeft, Pencil, Trash2 } from "lucide-react";
...
export function CommunityMessageBubble({
  ...
  const isOutgoing = isSameUserId(message.senderId, currentUserId);
  const deleted = Boolean(message.isDeleted);
  const canEditText =
    !deleted && (!message.messageType || message.messageType === "text");
  const showEdit = isOutgoing && onEditMessage && canEditText;
  const showDelete = isOutgoing && onDeleteMessage && !deleted;
  const showReply = !deleted && onReplyMessage;
    ...
    >
      <div
        className={cn(
          ...
          isOutgoing ? "bg-emerald-600" : "bg-[#0052CC]/25",
        )}
        aria-hidden
      />
      <div
      ...
      >
        <p
          className={cn(
            ...
            isOutgoing ? "text-emerald-700" : "text-foreground",
          )}
        >
          {message.sender}
        </p>
        ...
        >
          {message.replyPreview ? (
            <div
              className={cn(
                "mb-2 rounded-lg border-l-2 px-2 py-1.5 text-left text-xs",
                isOutgoing
                  ? "border-emerald-600 bg-emerald-100/70"
