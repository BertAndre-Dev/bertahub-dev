...
export interface ResidentAssetState {
...
const initialState: ResidentAssetState = {
  categories: [],
...
const residentAssetSlice = createSlice({
  name: "residentAsset",
  initialState,
  reducers: {
    clearResidentAssetError: (state) => {
      state.error = null;
    },
    clearCurrentResidentAsset: (state) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateAssetCategory.fulfilled, (state, action) => {
        ...
        const updated = (action.payload as { data?: AssetCategory })?.data;
      ...
      .addCase(createAssets.fulfilled, (state, action) => {
        ...
        const created = (action.payload as { data?: Asset | Asset[] })?.data;
      ...
      .addCase(updateAsset.fulfilled, (state, action) => {
        ...
        const updated = (action.payload as { data?: Asset })?.data;
        const id = (action.payload as { id?: string })?.id;
        const updatedId = getId(updated) || id;
        if (updatedId) {
...
export const { clearResidentAssetError, clearCurrentResidentAsset } =
  residentAssetSlice.actions;
export default residentAssetSlice.reducer;
