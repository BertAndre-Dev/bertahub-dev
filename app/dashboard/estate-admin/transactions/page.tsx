...
export default function TransactionPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // ✅ Fetch wallet using estateId instead of userId
  ...
  const handleCreateWallet = async () => {
    ...
    try {
      ...
      // Refresh wallet using estateId
      if (estateId) {
        dispatch(getWallet(estateId));
      }
    } catch (error: any) {
  ...
  const handleSumbit = async ({
    ...
    try {
      ...
      // Refresh wallet balance and transactions using estateId
      if (estateId) {
        await dispatch(getWallet(estateId));
        await dispatch(
          getEstateTransactionHistory({ estateId, page: currentPage, limit }),
        );
      }
  ...
  useEffect(() => {
    ...
    const verifyTransactionAsync = async () => {
      try {
        ...
        // Refresh wallet balance using estateId
