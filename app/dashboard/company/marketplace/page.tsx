...
} from "@/redux/slice/company/marketplace/company-marketplace";
...
export default function CompanyMarketplacePage() {
  ...
  const { list, pagination, getListStatus, createStatus, updateStatus } =
    useSelector((state: RootState) => {
      const s = state.companyMarketplace;
      return {
        list: s?.list ?? null,
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings.filter((item: MarketplaceItem) => {
  ...
  const handleSuspendConfirm = async (reason: string) => {
    ...
    try {
      await dispatch(
        suspendCompanyMarketplace({ marketPlaceId: suspendItem.id, reason }),
      ).unwrap();
  ...
  const renderEditTab = () => {
    ...
    return (
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filteredListings.map((item: MarketplaceItem) => (
  ...
  const renderViewTab = () => (
    <Table
      ...
      enableExport
      exportFileName="company-marketplace-businesses"
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground mt-1">Manage businesses in marketplace.</p>
        </div>
        <Button
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 xl:gap-2 bg-white p-4 rounded-lg border border-border">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="flex flex-row items-center gap-1">
            <label htmlFor="marketplace-status" className="text-sm text-muted-foreground">
              Status
            </label>
            <select
          ...
          <div className="flex flex-row items-center gap-1">
            <label htmlFor="marketplace-category" className="text-sm text-muted-foreground">
              Category
            </label>
            <input
        ...
        <Tab
          ...
          renderContent={(activeTab) => {
            if (activeTab === "Edit Business") {
              return <div className="space-y-4">{renderEditTab()}</div>;
            }
