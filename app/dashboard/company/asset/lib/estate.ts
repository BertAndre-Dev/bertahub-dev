...
export function mapCompanyEstateRows(
  rows: Array<{ id?: string; _id?: string; name?: string }> | undefined,
): EstateOption[] {
  ...
  return rows
    .map((e) => {
      ...
      if (!id) return null;
      return { id, name: e?.name ?? "Unnamed estate" };
    })
    .filter((x): x is EstateOption => Boolean(x));
}

/** Fallback when profile embeds estate(s); prefer getCompanyEstates API. */
export function parseCompanyEstates(data: Record<string, unknown>): EstateOption[] {
  ...
  const id =
    typeof rawEstateId === "string"
      ? rawEstateId
      : rawEstateId?._id || rawEstateId?.id || estateFromObj?._id || estateFromObj?.id || "";
  if (!id) return [];
