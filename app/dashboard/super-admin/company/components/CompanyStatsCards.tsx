...
import { Building2, Home, PowerOff, Layers } from "lucide-react";
...
export function CompanyStatsCards({
  ...
  const modulesEnabled = new Set(
    companies.flatMap((c) => (Array.isArray(c.modules) ? c.modules : [])),
  ).size;
  ...
  const stats = [
    ...
    { label: "Modules Enabled", value: modulesEnabled, icon: Layers, color: "bg-[#CABDFF80]" },
  ];
