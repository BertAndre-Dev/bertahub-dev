...
function resolveOrganizationName(data: Record<string, unknown>): string {
  ...
  const fallbackCompany = (data?.companyName as string) ?? "";
  const companyName = companyFromId || companyFromObj || fallbackCompany;
  if (companyName) return companyName;
  return extractEstateNameFromUser(data) ?? "your organization";
}

export default function EnergyProviderEstatePage() {
  ...
  const { allEstates, pagination, loading } = useSelector((state: RootState) => {
    const s = state.energyProviderEstate;
    return {
  ...
  const handleSubmitEstate = async (data: EstateData) => {
    try {
      ...
      if (id) {
        ...
        await dispatch(updateEnergyProviderEstate({ id, data: updateData })).unwrap();
        toast.success("Estate updated successfully!");
      } else {
        await dispatch(createEnergyProviderEstate(data)).unwrap();
  ...
  const handleConfirmStatus = async () => {
    ...
    try {
      if (statusMode === "suspend") {
        await dispatch(suspendEnergyProviderEstate(id)).unwrap();
        toast.info(`${estate?.name ?? "Estate"} has been suspended.`);
      } else {
        await dispatch(activateEnergyProviderEstate(id)).unwrap();
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <EnergyProviderEstateStatusModal
